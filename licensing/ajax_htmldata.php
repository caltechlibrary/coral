...
//
// ajax_htmldata.php formats display data for all pages - tables and search
// results
//
// when ajax_htmldata.php is called through ajax, 'action' parm is required
// to dictate which data will be returned
//
		...
		if (count($user->allAsArray()) > 0){
			...
			<table class='dataTable' style='width:550px'>
				...
				foreach($user->allAsArray() as $instance) {
					...
					echo "<td class='icon'><a href='ajax_forms.php?action=getAdminUserUpdateForm&loginID=" . $instance['loginID'] . "&height=210&width=295&modal=true' class='thickbox' id='expression'><img src='images/edit.gif' title= '"._("edit")."' /></a></td>";
